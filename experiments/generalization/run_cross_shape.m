...
assert(isfinite(splitId) && ismember(splitId, 1:12), ...
    'AFOP_SPLIT must be an integer from 1 to 12.');
data = load_base_data(cfg);
protocolFile = fullfile(cfg.paths.artifacts, sprintf('crossshape_split%d.mat', splitId));
...
fprintf('\n[Cross-shape] split=%d source_shapes=%s target_shapes=%s | stage=%s\n', ...
    splitId, mat2str(protocol.metadata.sourceShapes), ...
    mat2str(protocol.metadata.targetShapes), action);
...
output = fullfile(cfg.paths.runs, 'crossshape', sprintf('split_%d', splitId), ...
    'afop_eval', 'episodic_full.mat');
if isfile(output), fprintf('[AFOP eval] exists: %s\n', output); return; end
selection = cfg.dscan;
selection.seed = cfg.seed.train + splitId + 101;
...
save_new_artifact(output, struct('family', "AFOP", ...
    ...
    'trainingPolicy', "NCA and D-scan use source shapes only; target query is evaluation-only"));
...
targetShapeBlocks = [ ...
     ...
     4  6  7  8; ...
     1  3  5 10];
assert(all(arrayfun(@(shapeId) nnz(targetShapeBlocks == shapeId), 1:12) == 4), ...
    'Each shape must occur in exactly four target blocks.');
for repetition = 1:4
    rows = (repetition-1)*3 + (1:3);
    assert(isequal(sort(reshape(targetShapeBlocks(rows,:), 1, [])), 1:12), ...
        'Target blocks %d-%d do not form a complete partition.', rows(1), rows(end));
end

for splitId = 1:12
    targetShapes = sort(targetShapeBlocks(splitId,:));
    sourceShapes = setdiff(1:12, targetShapes);
    sourceRows = find(data.features.valid(:) & ...
        ismember(data.features.y_shape12(:), sourceShapes));
    targetRows = find(data.features.valid(:) & ...
        ismember(data.features.y_shape12(:), targetShapes));
    assert(numel(sourceRows) == 24*60, ...
        'Unexpected source row count for split %d.', splitId);
    assert(numel(targetRows) == 12*60, ...
        'Unexpected target row count for split %d.', splitId);

    stream = RandStream('mt19937ar', 'Seed', cfg.seed.fold + 100*splitId);
    sourceTrain = zeros(24*48, 1);
    sourceVal = zeros(24*12, 1);
    trainCursor = 0;
    valCursor = 0;
    sourceClasses = unique(data.features.y_class(sourceRows))';
    assert(numel(sourceClasses) == 24, ...
        'Eight source shapes must contain 24 shape-material classes.');
    ...
    assert(isequal(sort([fold.source_train; fold.source_val]), sort(fold.train_all)), ...
        'Source train/val do not cover the source shapes exactly.');
    assert(isempty(intersect(unique(data.features.y_shape12(fold.train_all)), ...
        unique(data.features.y_shape12(fold.test_all)))), ...
        'Source and target shapes overlap.');

    evalSeed = cfg.seed.eval + 10000*splitId;
    evalManifest = generate_episode_manifest(data.features, targetRows, combos, ...
        cfg.eval.episodes, evalSeed, "crossshape_shared_target_split" + splitId);
    metadata = struct();
    metadata.version = 1;
    metadata.splitId = splitId;
    metadata.sourceShapes = sourceShapes;
    metadata.targetShapes = targetShapes;
    metadata.repetition = ceil(splitId/3);
    metadata.foldWithinRepetition = mod(splitId-1,3) + 1;
    ...
    output = fullfile(cfg.paths.artifacts, sprintf('crossshape_split%d.mat', splitId));
    if isfile(output)
...
validationManifest = generate_episode_manifest(data.features, data.fold.source_val, ...
    ...
    cfg.seed.train + 1000*splitId + 1010, "crossshape_feat_pretrain_validation");
...
validationManifest = generate_episode_manifest(data.features,data.fold.source_val, ...
    ...
    cfg.seed.train + 1000*splitId + 2020 + shot,"crossshape_feat_meta_validation");
...
output=fullfile(cfg.paths.runs,'crossshape',sprintf('split_%d',splitId),'feat_eval','episodic_full.mat');
...
validationManifest=generate_episode_manifest(data.features,data.fold.source_val,[5 1 5], ...
    cfg.mamlpp.validationEpisodes,cfg.seed.train+1000*splitId+3030,"crossshape_mamlpp_validation");
...
output=fullfile(cfg.paths.runs,'crossshape',sprintf('split_%d',splitId),'mamlpp_eval','episodic_full.mat');
...
runDir=fullfile(cfg.paths.runs,'crossshape',sprintf('split_%d',splitId),char(family), ...
...
cfg.paths.runs=fullfile(originalRuns,'crossshape',sprintf('split_%d',splitId));
ensure_dir(cfg.paths.runs);
runInfo=create_run_directory(cfg,modelName,stage);
end

function path=stage_pointer(cfg,splitId,name)
path=fullfile(cfg.root,'state',sprintf('crossshape_split%d_%s.mat',splitId,name));
end
